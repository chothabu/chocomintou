...
final class SearchViewModel {
    ...
    var stores: [StorePin] = []
    /// 取り扱いを確認できたチェーンと、その商品。
    var offerings: [ChainOffering] = []
    var isLoading = false
    ...
    func search(services: AppServices, coordinate: CLLocationCoordinate2D?) async {
        ...
        do {
            ...
            case .stores:
                ...
                await loadOfferings(services: services, keyword: keyword)
            }
        } catch {
    ...
    /// 取り扱いを確認できたチェーンと商品を読む。
    ///
    /// チェーン単位の事実なので、現在地とは無関係に出せる。
    /// 「この店に今ある」ではなく「このチェーンで売っている」を伝える。
    private func loadOfferings(services: AppServices, keyword: String) async {
        let all = (try? await services.stores.chainOfferings()) ?? []
        offerings = keyword.isEmpty
            ? all
            : all.filter { offering in
                offering.chainName.localizedCaseInsensitiveContains(keyword)
